...
def interaction_boundary(mol,F,Fg, num_H = 1, num_O = 5, K = 10):
    atoms = mol.atom_numbers()
    new_F = Fg.copy()
    coords = mol.coords
    mapping = [0, num_H,num_H, num_O,num_O,num_O,num_O,num_O,num_O,num_O,num_O]
    for i in range(len(mol.atoms)):
        supplementary = []
        interacts = []
        start_i = np.sum([mapping[x] for x in atoms[:i]])
        end_i = np.sum([mapping[x] for x in atoms[:i+1]])
        for j in range(len(mol.atoms)):
            #if i != j:
            start_j = np.sum([mapping[x] for x in atoms[:j]])
            end_j = np.sum([mapping[x] for x in atoms[:j+1]])
            cache2 = [start_i, end_i,start_j, end_j]
            cache = [np.linalg.norm(coords[i]-coords[j]),-atoms[j]]
            supplementary.append(cache2)
            interacts.append(cache)
        interacts = np.array(interacts)
        supplementary = np.array(supplementary)
        order = argsort(interacts)
        order = order[:K+1]
        for j in order:
            cache = supplementary[j]
            start_i = int(cache[0])
            end_i = int(cache[1])
            start_j = int(cache[2])
            end_j = int(cache[3])
            new_F[start_i:end_i,start_j:end_j] = F[start_i:end_i,start_j:end_j]
            new_F[start_j:end_j,start_i:end_i] = F[start_j:end_j,start_i:end_i]
    return new_F

def extract_interaction_data(mol,M, num_H = 1, num_O = 5, K = 10):
    atoms = mol.atom_numbers()
    coords = mol.coords
    mapping = [0, num_H,num_H, num_O,num_O,num_O,num_O,num_O,num_O,num_O,num_O]
    data = []
    for i in range(len(mol.atoms)):
        ...
        for j in range(len(mol.atoms)):
            ...
            start_j = np.sum([mapping[x] for x in atoms[:j]])
            end_j = np.sum([mapping[x] for x in atoms[:j+1]])
            cache = [norm_dist,-atoms[j]]
            cache2 = [start_i, end_i,start_j, end_j,norm_dist,dist/norm_dist, atoms[i],atoms[j]]
            interacts.append(cache)
            supplementary.append(cache2)
        interacts = np.array(interacts)
        order = argsort(interacts)
        ...
        for j in order:
            ...
            start_j = int(cache[2])
            end_j = int(cache[3])
            data[i]['matrix_block'].append(M[start_i:end_i,start_j:end_j])
            data[i]['distance'].append(cache[4])
            data[i]['rotation'].append(cache[5])
            data[i]['type'].append(np.array([cache[6],cache[7]]))
    return data
