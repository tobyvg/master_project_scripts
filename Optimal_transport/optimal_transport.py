...
def ot_movie(a,b,in_gamma, frames = 5,name = 'Animation', transform = transform,interval = 1000, repeat_delay = 3000, animate = True, save = False):
    start_time = time.time()
    frames += 1
    mini = a.min()
    maxi = a.max()
    assert a.shape == b.shape, "Input images of different shape are not supported"
    if save:
        results = []
    ...
    if animate:
        fig = plt.figure()
        plts = []
        p = plt.imshow(a, cmap = mpl.cm.Greys,vmin = mini,vmax= maxi)
        plts.append( [p] )
    calc_time = 0
    indices_provided = False
    indices = np.zeros((np.prod(a.flatten().shape),2))
    for i in range(1,frames):
        calc = time.time()
        intermediate = transform(a.flatten(),np.array(a.shape),in_gamma,i,frames,indices = indices,indices_provided = indices_provided)
        indices = intermediate[1]
        intermediate = intermediate[0].reshape(a.shape)
        calc = time.time() - calc
        if save:
        ...
        calc_time += calc
        if animate:
            p = plt.imshow(intermediate, cmap = mpl.cm.Greys,vmax = maxi,vmin = mini)
            plts.append( [p] )
    if animate:
        p = plt.imshow(b, cmap = mpl.cm.Greys,vmin = mini,vmax= maxi)
        plts.append( [p] )
        ani = animation.ArtistAnimation(fig, plts, interval=interval/(frames/10), repeat_delay=repeat_delay)
        ani.save(name + '.mp4')
        plt.clf()
    total_time = time.time() - start_time
    print('Execution time: ' + str(total_time))
    print('Percentage of time spent on creating intermediate images: ' + str(calc_time/total_time*100) + ' %')
    if animate:
        print('Saved as: ' + name + '.mp4')
    if save:
